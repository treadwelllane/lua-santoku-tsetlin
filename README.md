...
|----------|------------|---------|-------------|
| `corex.create` | `table: {visible, hidden, lam?, spa?, tmin?, ttc?, anchor?, tile_s?, tile_v?, threads?}` | `userdata: tk_corex_t` | Create CorEx model with visible/hidden dimensions |
| `corex.load` | `string|userdata: data, boolean?: from_string, number?: threads` | `userdata: tk_corex_t` | Load CorEx model from file path or serialized data |
| `corex:train` | `table: {corpus, samples, iterations, each?}` | `nil` | Train CorEx model on corpus data |
| `corex:compress` | `userdata: set_bits, number?: n_samples` | `userdata: tk_ivec_t` | Compress data using trained model |
| `corex:top_visible` | `number: top_k` | `userdata: tk_ivec_t` | Extract top-k visible features |
...
| `ann:add` | `string: data, number|userdata: base_id|ids, number?: n_nodes` | `-` | Add binary vectors with node IDs |
| `ann:remove` | `number: id` | `-` | Remove node by ID |
| `ann:get` | `number: id` | `string?` | Get binary vector by ID |
| `ann:neighborhoods` | `number?: k, number?: eps, number?: min, boolean?: mutual` | `userdata: tk_hoods_t, userdata: tk_ivec_t` | Find k-nearest neighborhoods within Hamming distance eps |
| `ann:neighbors` | `number|string: id|vector, number?: knn, number?: eps, userdata?: out` | `userdata: tk_pvec_t` | Find neighbors for ID or vector within distance eps |
...
| `ann:ids` | `-` | `userdata: tk_ivec_t` | Return all node IDs |
...
| `inv:add` | `userdata: features, userdata: ids` | `-` | Add sparse feature vectors with node IDs |
| `inv:remove` | `number: id` | `-` | Remove node by ID |
| `inv:get` | `number: id` | `table?` | Get feature vector by ID |
| `inv:neighborhoods` | `number?: k, number?: eps, number?: min, string?: cmp, number?: alpha, number?: beta, boolean?: mutual` | `userdata: tk_hoods_t, userdata: tk_ivec_t` | Find neighborhoods using similarity metrics |
| `inv:neighbors` | `number|userdata: id|vector, number?: knn, number?: eps, string?: cmp, number?: alpha, number?: beta, userdata?: out` | `userdata: tk_rvec_t` | Find neighbors with similarity scores |
...
| `inv:ids` | `-` | `userdata: tk_ivec_t` | Return all node IDs |
...
| `hbi:add` | `string: data, number|userdata: base_id|ids, number?: n_nodes` | `-` | Add binary data with node IDs |
| `hbi:remove` | `number: id` | `-` | Remove node by ID |
| `hbi:get` | `number: id` | `string?` | Get binary data by ID |
| `hbi:neighborhoods` | `number?: k, number?: eps, number?: min, boolean?: mutual` | `userdata: tk_hoods_t, userdata: tk_ivec_t` | Find neighborhoods within Hamming radius |
| `hbi:neighbors` | `number|string: id|vector, number?: knn, number?: eps, userdata?: out` | `userdata: tk_pvec_t` | Find neighbors within Hamming distance |
...
| `hbi:ids` | `-` | `userdata: tk_ivec_t` | Return node IDs |
...
| `dataset.random_pairs` | `userdata: ids, number?: edges_per_node` | `userdata: tk_pvec_t` | Generate random node pairs for graph construction |
| `dataset.anchor_pairs` | `userdata: ids, number?: n_anchors` | `userdata: tk_pvec_t` | Generate anchor-based node pairs |
| `dataset.classes_index` | `userdata: ids, userdata: classes` | `userdata: inv_t` | Create class-based inverted index |
| `dataset.multiclass_pairs` | `userdata: ids, userdata: labels, number?: n_anchors_pos, number?: n_anchors_neg, userdata?: index, number?: eps_pos, number?: eps_neg` | `userdata: tk_pvec_t, userdata: tk_pvec_t` | Generate positive and negative pairs for classification |
...
| `graph:pairs` | `-` | `userdata: tk_pvec_t, userdata: tk_dvec_t` | Get edge pairs and weights |
| `graph:adjacency` | `-` | `userdata: tk_ivec_t, userdata: tk_ivec_t, userdata: tk_ivec_t, userdata: tk_dvec_t` | Get CSR adjacency representation: node IDs, offsets, neighbors, weights |
...
| Function | Arguments | Returns | Description |
|----------|------------|---------|-------------|
| `itq.encode` | `table: {codes, n_dims, iterations?, tolerance?, threads?, each?}` | `userdata: tk_ivec_t` | Apply ITQ to continuous embeddings |
| `itq.sign` | `table: {codes, n_dims}` | `userdata: tk_ivec_t` | Simple sign-based binarization |
| `itq.median` | `table: {codes, n_dims}` | `userdata: tk_ivec_t` | Median-based binarization |
...
| Function | Arguments | Returns | Description |
|----------|------------|---------|-------------|
| `spectral.encode` | `table: {ids, offsets, neighbors, weights, n_hidden, threads?, normalized?, eps_primme?, each?}` | `userdata: tk_ivec_t, userdata: tk_dvec_t` | Compute spectral embeddings from graph adjacency |
...
| Function | Arguments | Returns | Description |
|----------|------------|---------|-------------|
| `tch.refine` | `table: {codes, ids, offsets, neighbors, weights, n_dims, each?}` | `userdata: tk_ivec_t` | Refine binary codes by minimizing graph energy |
...
| `booleanizer:encode` | `sample_id, feature, value OR dvec, n_dims, id_dim0?, out?` | `userdata: tk_ivec_t` | Encode features to binary representation |
...
| `booleanizer:restrict` | `userdata: tk_ivec_t` | `-` | Restrict to subset of features |
...
| `tokenizer:tokenize` | `string|table: text, userdata?: out` | `userdata: tk_ivec_t` | Tokenize text to feature indices |
...
| `tokenizer:restrict` | `userdata: top_vocab` | `-` | Restrict to top vocabulary items |
