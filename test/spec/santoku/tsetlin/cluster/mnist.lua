...
test("tsetlin", function ()
  ...
  dataset.similarity = eval.optimize_retrieval({
    ...
    each = function (a, f, p, r, m)
      local d, dd = stopwatch()
      str.printf("  Time: %6.2f %6.2f | AUC: %6.2f | F1: %.2f | Precision: %.2f | Recall: %.2f | Margin: %d\n", -- luacheck: ignore
        d, dd, a, f, p, r, m)
    end
  })
  str.printi("\n  Best | AUC: %.2f#(auc) | F1: %.2f#(f1) | Precision: %.2f#(precision) | Recall: %.2f#(recall) | Margin: %d#(margin)", -- luacheck: ignore
    dataset.similarity)
  ...
  dataset.index_codes = hbi.create({ features = dataset.n_hidden })
  ...
  dataset.cluster_score, dataset.cluster_ids, dataset.cluster_assignments, dataset.n_clusters = eval.optimize_clustering({ -- luacheck: ignore
    ...
    each = function (f, p, r, m, c)
      local d, dd = stopwatch()
      str.printf("  Time: %6.2f %6.2f | F1: %.2f | Precision: %.2f | Recall: %.2f | Margin: %d | Clusters: %d\n", -- luacheck: ignore
        d, dd, f, p, r, m, c)
    end
  })
  dataset.cluster_score.n_clusters = dataset.n_clusters
  str.printi("\n  Best | F1: %.2f#(f1) | Precision: %.2f#(precision) | Recall: %.2f#(recall) | Margin: %d#(margin) | Clusters: %d#(n_clusters)\n", -- luacheck: ignore
