...
local cfg; cfg = {
  early_exit_after = nil,
  lookup_mode = "token",
  aggregate_mode = "supervised",
  data = {
    max_per_class = nil,
    n_classes = 20,
    tvr = 0.1,
...
test("newsgroups-landmarks", function()
  ...
  local train_encoder_input = encode_landmarks(train_pred_features, train.n)
  train_encoder_input:bits_flip_interleave(n_latent)
  local test_encoder_input = encode_landmarks(test_pred_features, test.n)
  test_encoder_input:bits_flip_interleave(n_latent)
  local validate_encoder_input = encode_landmarks(validate_pred_features, validate.n)
