...
static inline void tk_hbi_add (
  ...
  for (uint64_t i = 0; i < ids->n; i ++) {
    ...
    tk_ivec_t *bucket;
    // tk_hbi_code_t h = (tk_hbi_code_t) 0;
    // memcpy(&h, data + i * ((A->features + CHAR_BIT - 1) / CHAR_BIT), ((A->features + CHAR_BIT - 1) / CHAR_BIT));
    tk_hbi_code_t h = 0;
    size_t nbytes = (A->features + CHAR_BIT - 1) / CHAR_BIT;
    memcpy(&h, data + i * nbytes, nbytes);
...
static inline void tk_hbi_populate_neighborhood (
  ...
  tk_hbi_code_t h = (tk_hbi_code_t) 0;
  memcpy(&h, V, ((A->features + CHAR_BIT - 1) / CHAR_BIT));
  tk_hbi_extend_neighborhood(A, i, sid, V, h, hood, sid_idx, k, 0);
  if (k && hood->n >= k)
    return;
  int pos[TK_HBI_BITS];
  for (int r = 1; r <= (int) eps && r <= TK_HBI_BITS; r ++) {
    for (int i = 0; i < r; i ++)
      pos[i] = i;
    while (true) {
...
static inline void tk_hbi_neighborhoods (
  ...
  tk_threads_signal(A->pool, TK_HBI_NEIGHBORHOODS);
...
static inline tk_pvec_t *tk_hbi_neighbors_by_vec (
  ...
  out->n = 0;
  tk_hbi_code_t h0 = (tk_hbi_code_t) 0;
  memcpy(&h0, vec, ((A->features + CHAR_BIT - 1) / CHAR_BIT));
  int pos[TK_HBI_BITS];
  for (int r = 0; r <= (int) eps && r <= TK_HBI_BITS; r ++) {
    for (int i = 0; i < r; i ++)
      pos[i] = i;
    bool done = (r == 0);
    while (true) {
...
static inline tk_pvec_t *tk_hbi_neighbors_by_id (
  ...
  return tk_hbi_neighbors_by_vec(L, A, tk_hbi_get(A, uid), sid0, knn, eps, out);
}
...
static inline int tk_hbi_neighbors_lua (lua_State *L)
{
  ...
  uint64_t eps = tk_lua_optunsigned(L, 5, "eps", 0);
  tk_pvec_t *out = tk_pvec_peekopt(L, 5);
  if (lua_type(L, 2) == LUA_TNUMBER) {
    int64_t uid = tk_lua_checkinteger(L, 2, "id");
    tk_hbi_neighbors_by_id(L, A, uid, knn, eps, out);
  } else {
    char *vec = (char *) tk_lua_checkustring(L, 2, "vector");
    tk_hbi_neighbors_by_vec(L, A, vec, -1, knn, eps, out);
  }
  return out == NULL ? 1 : 0;
}
