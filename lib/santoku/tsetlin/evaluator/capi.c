...
static inline int tk_cluster_centroid(
  ...
  tk_cluster_new_t **clusters = malloc(n_nodes * sizeof(tk_cluster_new_t*));
  ...
  for (uint64_t i = 0; i < n_nodes; i++) {
    ...
    if (kha) {
      ...
      while (chain_idx != -1) {
        tk_cluster_new_t *existing = clusters[chain_idx];
    ...
    if (need_new_cluster) {
      tk_cluster_new_t *cluster = malloc(sizeof(tk_cluster_new_t));
      if (!cluster) {
    ...
    tk_cluster_new_t *cluster = clusters[cluster_idx];
  ...
  while (edge_heap->n > 0 && n_active > 1) {
    ...
    while (batch_start < distance_level_edges->n) {
      ...
      for (uint64_t edge_idx = batch_start; edge_idx < batch_end; edge_idx++) {
        ...
        tk_cluster_new_t *ci = clusters[ci_idx];
        tk_cluster_new_t *cj = clusters[cj_idx];
  ...
  for (uint64_t i = 0; i < n_clusters; i++) {
    tk_cluster_new_t *cluster = clusters[i];
    if (cluster) {
      if (cluster->members) tk_ivec_destroy(cluster->members);
      if (cluster->centroid) tk_centroid_destroy(cluster->centroid);
      if (cluster->neighbor_ids) tk_iuset_destroy(cluster->neighbor_ids);
...
static inline int tm_optimize_clustering (lua_State *L)
{
  ...
  uint64_t n_steps = dendro_offsets->n > n_samples + 1 ?
  ...
  lua_setfield(L, -2, "n_clusters");
  lua_pushinteger(L, (lua_Integer) n_steps);
...
static double tk_compute_reconstruction (
  ...
  #pragma omp parallel reduction(+:total) reduction(+:total_weight) reduction(+:total_nodes_processed)
  {
    ...
    if (!bin_ranks || !rank_buffer_b || !count_buffer || !avgrank_buffer || !itmp) {
      ...
      for (uint64_t node_idx = 0; node_idx < n_nodes; node_idx++) {
        ...
        if (metric_alpha >= 0.0) {
          uint64_t n_neighbors = (uint64_t)(end - start);
          double weight = pow((double)n_neighbors, metric_alpha);
          total += corr * weight;
          total_weight += weight;
        } else {
          total += corr;
  ...
  if (start_prefix < 0) {
    ...
    for (uint64_t k = min_prefix; k <= n_dims; k ++) {
      ...
      double score = tk_compute_reconstruction(state->codes, state->ann, state->hbi, state->adjacency_ids,
        state->n_dims, state->offsets, state->neighbors, state->weights, state->eval_metric, state->metric_alpha, mask, k);
    ...
    for (uint64_t b = 0; b < n_dims; b ++) {
      ...
      mask[TK_CVEC_BITS_BYTE(b)] |= (1 << TK_CVEC_BITS_BIT(b));
      double score = tk_compute_reconstruction(state->codes, state->ann, state->hbi, state->adjacency_ids,
        state->n_dims, state->offsets, state->neighbors, state->weights, state->eval_metric, state->metric_alpha, mask, 1);
      if (score > best_prefix_score + 1e-12) {
        best_prefix_score = score;
        best_bit = b;
      }
    ...
    best_prefix_score = tk_compute_reconstruction(state->codes, state->ann, state->hbi, state->adjacency_ids,
      state->n_dims, state->offsets, state->neighbors, state->weights, state->eval_metric, state->metric_alpha, mask, best_prefix);
    if (i_each >= 0) {
      lua_pushvalue(L, i_each);
      lua_pushinteger(L, (lua_Integer) best_prefix);
      lua_pushnumber(L, 0.0);
  ...
  while (improved && active->n > 0) {
    ...
    if (active->n > 1)
      ...
      for (uint64_t i = 0; i < active->n; i ++) {
        ...
        double score = tk_compute_reconstruction(state->codes, state->ann, state->hbi, state->adjacency_ids,
          state->n_dims, state->offsets, state->neighbors, state->weights, state->eval_metric, state->metric_alpha, candidate, active->n - 1);
