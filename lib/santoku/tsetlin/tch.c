...
static inline int tk_tch_refine_lua (lua_State *L)
{
  ...
  uint64_t max_iterations = tk_lua_foptunsigned(L, 1, "tch", "iterations", 1000);
  int i_each = -1;
  if (tk_lua_ftype(L, 1, "each") != LUA_TNIL) {
    lua_getfield(L, 1, "each");
    i_each = tk_lua_absindex(L, -1);
  }
  tk_tch_refine(L, codes, uids, adj_offset, adj_data, adj_weights, n_dims, knn, max_iterations, i_each);
